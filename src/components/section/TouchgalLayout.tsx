...
export const TouchgalLayout = ({ posts, widgets, totalPostsCount }: { posts: Post[], widgets: any, totalPostsCount?: number }) => {
    ...
    <div className="mx-auto block max-w-7xl px-4 md:px-8 lg:px-12">
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 mb-16">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
            <Link 
              key={pageNum} 
              href={pageNum === 1 ? '/' : `/${ARCHIVE}/${pageNum}`}
              className={`
                w-10 h-10 flex items-center justify-center rounded-xl font-black text-sm transition-all duration-300
                ${pageNum === 1 
                  ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/40 scale-110' 
                  : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-400 hover:bg-blue-500/20 hover:text-blue-500'
                }
              `}
            >
              {pageNum}
          ...
          {/* 如果页数极多，显示一个“查看全部” */}
          {totalPages > 5 && (
            <Link href={`/${ARCHIVE}/1`} className="px-4 h-10 flex items-center justify-center rounded-xl bg-neutral-100 dark:bg-neutral-800 text-neutral-400 font-bold text-xs hover:text-blue-500 transition-colors">
              全部 →
            </Link>
          )}
        </div>
      ...
      <div className="bg-neutral-100 dark:bg-neutral-900 rounded-[2.5rem] p-12 border border-neutral-200 dark:border-neutral-800 mb-16 text-center relative overflow-hidden">
         ...
         <div className="flex flex-wrap justify-center gap-6 relative z-10">
            <button onClick={() => window.open('https://van.pro-plus.top/dashboard', '_blank')} className="px-10 py-4 bg-[#A855F7] text-white rounded-2xl text-sm font-black hover:bg-[#9333EA] transition-all hover:scale-105 active:scale-95 shadow-xl shadow-purple-500/40 flex items-center gap-2">
              <span>🛒</span> 前往贩售机
            </button>
            <Link href="/about" className="px-10 py-4 bg-[#F97316] text-white rounded-2xl text-sm font-black hover:bg-[#EA580C] transition-all hover:scale-105 active:scale-95 shadow-xl shadow-orange-500/40 flex items-center gap-2">
              <span>👑</span> 资源介绍
            </Link>
         </div>
      </div>
